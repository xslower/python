...
def precise_regress(y, y_pred):
    ...
    for i in y:
        ...
        #     cnt[1] += 1
        if len(zero_vals) == 0:
            zero_vals.append([y_pred[i], 1, y[i]])
        else:
            found = False
            for j in range(len(zero_vals)):
                val = zero_vals[j][0]
                n = zero_vals[j][1]
                ev = zero_vals[j][2]
                if val - 0.1 < y_pred[i] < val + 0.1:
                    found = True
                    zero_vals[j][1] += 1
                    zero_vals[j][2] = (ev * n + y[i]) / (n + 1)
            if not found:
                zero_vals.append([y_pred[i], 1, y[i]])
...
def main():
    ...
    dx, dy = init_data()
    x_train, x_test, y_train, y_test = train_test_split(dx, dy, 5)
    svc = my_svc()
