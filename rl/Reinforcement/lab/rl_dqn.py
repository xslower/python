...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l2'):
                ...
                self.q_learn = tf.matmul(l1, w2) + b2
    ...
    def learn(self):
        ...
        # q_predict, q_learn = self.sess.run(
        #     [self.q_predict, self.q_learn],
        #     feed_dict={
        #         self.s_: batch_memory[:, -self.n_features:],  # fixed params
        #         self.s: batch_memory[:, :self.n_features],  # newest params
        #     })
        q_predict = self.sess.run(self.q_predict, feed_dict={self.s_: batch_memory[:, -self.n_features:]})
        ...
        choosed_act_index = batch_memory[:, self.n_features].astype(int)
        reward = batch_memory[:, self.n_features + 1]

        # 这里只更新样本选择过的act的reward，其它的不管
        q_target[batch_indexes, choosed_act_index] = reward + self.gamma * np.max(q_predict, axis=1)
        """
