...
def dicriminator(z_real, z_gen):
    act = lrelu
    with tf.variable_scope('dis'):
        z = tf.concat([z_real, z_gen], axis=0)
        z = tf.layers.dense(z, units=n_latent, activation=act)
        z = tf.layers.dense(z, units=n_latent, activation=act)
        y = tf.layers.dense(z, units=1, activation=tf.nn.sigmoid)
        y_real, y_gen = tf.split(y, 2, axis=0)

        return y_real, y_gen


def decoder(z_gen, keep_prob, z_label = None):
...
z_gened = ae_encoder(X_in)
dec = decoder(z_gened, keep_prob, Y)
y_real, y_gen = dicriminator(Z_in, z_gened)

x_gen_flat = tf.reshape(dec, [-1, 28 * 28])
...
d_loss = -tf.reduce_mean(tf.log(y_real) + tf.log(1 - y_gen))
...
ae_trainer = optimizer.minimize(img_loss, var_list=dec_para + enc_para)
d_trainer = optimizer.minimize(d_loss, var_list=dis_para)
g_trainer = optimizer.minimize(g_loss, var_list=enc_para)
...
for i in range(3000):
    ...
    sess.run([ae_trainer, d_trainer, g_trainer], feed_dict={X_in: x_value, Y: c_value, Z_in: z_samples, keep_prob: 0.8})
    ...
    if i % 50 == 0:
        ...
        imgs = sess.run(dec, feed_dict={z_gened: z_samples, Y: c_sample_val, keep_prob: 1.0})
